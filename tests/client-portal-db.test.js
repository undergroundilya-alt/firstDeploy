...
(async()=>{
  ...
  try{
    ...
    res=await fetch(`${base}/account`,{headers:{cookie:cliCookie}}); assert.equal(res.status,200); html=await res.text(); assert.match(html,/Account/); assert.match(html,/Create project/); const token=csrf(html); console.log('   ✓ account page opens with client cookie and CSRF');
    res=await fetch(`${base}/account/projects/create`,{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded',cookie:cliCookie},body:form({csrf:token,projectName:'Static Banner Test',domain:'example.com',adContainerSelector:'[data-adproof-slot="left-sidebar"]',protectedSelector:'[data-adproof-content="protected"]'}),redirect:'manual'}); assert.equal(res.status,302); assert.ok((res.headers.get('location') || '').startsWith('/account')); console.log('   ✓ client can create a project linked to account');
